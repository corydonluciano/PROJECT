...
class MeditationTimerWindow(QWidget):
    def __init__(self, duration, music_file, title):
        ...
        # Начальный текст
        self.text_label = QLabel("Закрой глаза и сделай глубокий вдох через нос, задержи дыхание на несколько секунд, затем медленно выдохни через рот. Почувствуй, как тело расслабляется, а ум успокаивается. Позволь себе отпустить все мысли и заботы этого дня. Ты здесь, в этом моменте, и ничто другое не имеет значения.")
    ...
    def update_timer(self):
        ...
        if self.time_left == 30:  # Середина медитации
            self.text_label.setText("Продолжай дышать глубоко и ровно. Если мысли начинают блуждать, мягко верни внимание к дыханию. Ощущай каждый вдох и выдох, как они наполняют тебя энергией и спокойствием. Позволь своему телу быть тяжелым и неподвижным, словно оно сливается с землей под тобой.")
        elif self.time_left == 0:  # Конец медитации
            self.timer.stop()
            self.timer_label.setText(f"Медитация закончилась")
            self.text_label.setText("Постепенно возвращайся к осознанию своего тела. Сделай еще пару глубоких вдохов и выдохов. Когда будешь готов, открой глаза. Посмотри вокруг себя с новым восприятием, почувствуй благодарность за этот момент тишины и покоя. Пусть это состояние останется с тобой на протяжении всего дня.")
